 ...
 */
 ...
 function universaal_theme_comment( $comment, $args, $depth ) {
	 //проверяем в каком стиле у нас родитель (ol, ul, div)
	if ( 'div' === $args['style'] ) {
		//если стиль- div, то тег div
		$tag       = 'div';
		$add_below = 'comment';
	} else {
  ...
  <div class="comment-content">
	...
	<div class="comment-reply">
		<svg width="14" height="14" fill="#fff" class="icon comments-add-icon">
	...
	if ( have_comments() ) :
		...
		<div class="comments-header">
			...
			<a href="#commentform" class="comments-add-button">
				<svg width="18" height="18" fill="#fff" class="icon comments-add-icon">
		...
		<ol class="comments-list">
			...
			wp_list_comments(
				array(
					...
					'short_ping' => true,
					'avatar_size' => 75,
					'callback' => 'universaal_theme_comment',
	...
	comment_form(array(
		...
		<div class="comment-wrapper">	
			' . get_avatar(get_current_user_id(), 75) . '
				<div class="comment-textarea-wrapper"><textarea class="comment-textarea" id="comment" name="comment" aria-required="true"></textarea></div>
			</div>
		</div>',
